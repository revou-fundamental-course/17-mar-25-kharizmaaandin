...
function hitungLuas() {
  ...
  let hasil = sisi * sisi;
  document.getElementById("hasilLuas").innerHTML = `<p>L = S x S</p>
         <p>L = ${sisi} x ${sisi}</p>
         <p>L = ${hasil}</p>`;
}
...
function hitungKeliling() {
  ...
  // Panggil validasi sebelum perhitungan
  if (!validateInput(sisi)) return;

  let keliling = 4 * sisi;
  document.getElementById("hasilKeliling").innerHTML = `<p>K = 4 x S</p>
         <p>K = 4 x ${sisi}</p>
         <p>K = ${keliling}</p>`;
}
